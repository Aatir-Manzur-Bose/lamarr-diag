...
int jwClose( JWC_DECL0 )
{
	...
	if( JWC(error) != JWRITE_OK ) {
	  diag_verbose(VERBOSE, verbosity, "%s\n", "Error in jwCLose");
	  ErrorDisplay (JWC(error));
	}
	return JWC(error);
}
...
{
  jwObj_raw( JWC_PARAM key, (oneOrZero) ? (char *)"true" : (char *)"false" );
}

void jwObj_null( JWC_DECL char *key )
{
  jwObj_raw( JWC_PARAM key, (char *)"null" );
}
...
void jwArr_raw_json_int( JWC_DECL char *key, char *rawtext )
{
  if(_jwArr( JWC_PARAM0 ) == JWRITE_OK) {
...
void jwArr_double_json_int( JWC_DECL char *key, double value )
{
	modp_dtoa2( value, JWC(tmpbuf), 6 );
	jwArr_raw_json_int( JWC_PARAM key, JWC(tmpbuf) );
}
void jwArr_int_json_int( JWC_DECL char *key, int value )
{
	modp_itoa10( value, JWC(tmpbuf) );
	jwArr_raw_json_int( JWC_PARAM key, JWC(tmpbuf) );
}
void jwArr_array_json_int( JWC_DECL char *key )
{
	if(_jwArr( JWC_PARAM0 ) == JWRITE_OK)
	{
	  jwPutstr( JWC_PARAM key);
...
{
  jwArr_raw( JWC_PARAM  (oneOrZero) ? (char *)"true" : (char *)"false" );
}

void jwArr_null( JWC_DECL0 )
{
  jwArr_raw( JWC_PARAM  (char *)"null" );
}
...
char *jwErrorToString( int err )
{
	switch( err )
	{
	case JWRITE_OK:         return (char *)"OK"; 
	case JWRITE_BUF_FULL:   return (char *)"output buffer full";
	case JWRITE_NOT_ARRAY:	return (char *)"tried to write Array value into Object";
	case JWRITE_NOT_OBJECT:	return (char *)"tried to write Object key/value into Array";
	case JWRITE_STACK_FULL:	return (char *)"array/object nesting > JWRITE_STACK_DEPTH";
	case JWRITE_STACK_EMPTY:return (char *)"stack underflow error (too many 'end's)";
	case JWRITE_NEST_ERROR:	return (char *)"nesting error, not all objects closed when jwClose() called";
	}
...
void jwPutch( JWC_DECL char c )
{
	if( (unsigned int)(JWC(bufp) - JWC(buffer)) >= JWC(buflen) )
	{
		...
		diag_verbose(VERBOSE, verbosity, "error jwOpen buffer overrun buflen=%d\n", JWC(buflen));
		exit (EXIT_FAILURE);
	}else{
		*JWC(bufp)++ = c;
	}
...
int jWriteFileReset (char *pFileName, char *pStr) {
  char gp_str [GP_STR_LEN];
  FILE *p_file=0;
  snprintf (gp_str, GP_STR_LEN, "%s/%s",  RDW_TMP_DIR,pFileName);
  p_file = fopen (gp_str,"w");
  if (NULL==p_file) {
    diag_verbose(VERBOSE, verbosity, "error opening file: %s, exiting\n", gp_str);
    return (0);
  }
  if (pStr) fprintf (p_file, "%s", pStr);
  if (p_file) fclose (p_file);
  return (!0);
}
int jWriteFile ( JWC_DECL char *pFileName) {
  char gp_str [GP_STR_LEN];
  FILE *p_file=0;
  snprintf (gp_str, GP_STR_LEN, "%s/%s",  RDW_TMP_DIR,pFileName);

  diag_verbose(VERBOSE, verbosity, "writing %s to file %s\n", JWC(buffer), gp_str);
  ...
  if (JWC(buffer)) fprintf (p_file, "%s", JWC(buffer));
  if (p_file) fclose (p_file);
  //!<<<< reset buffer pointer
  JWC(bufp) = JWC(buffer);
  memset (JWC(buffer), 0, JWC(buflen));
  return (!0);
}
void ErrorDisplay (int err) {
  diag_verbose(VERBOSE, verbosity, "JWrite Error: %s\n",jwErrorToString(err));
}
