...
char * jReadCountObject( char *pJson, struct jReadElement *pResult, int keyIndex )
{
	...
	}else
	{
		while( 1 )
		{
			...
			pJson= jRead( ++pJson, (char *)"", &jElement );
			if( pResult->error )
				break;
			pJson= jReadFindTok( pJson, &jTok );
			pResult->elements++;
...
char * jReadCountArray( char *pJson, struct jReadElement *pResult )
{
	...
	}else
	{
		while( 1 )
		{
		  pJson= jRead( ++pJson, (char *)"", &jElement );	// array value
			if( pResult->error )
				break;
...
char *jReadArrayStep( char *pJsonArray, struct jReadElement *pResult )
{
	...
	case JREAD_COMMA:	// element separator
	  return jRead( ++pJsonArray, (char *)"", pResult );
...
char * jReadParam( char *pJson, char *pQuery, struct jReadElement *pResult, int *queryParams )
{
	...
	case JREAD_OBJECT:		// "{"
		...
		while( 1 )
		{
			...
			pJson= jRead( ++pJson, (char *)"", pResult );
			pJson= jReadFindTok( pJson, &jTok );
			if( jTok == JREAD_EOBJECT )
			{
...
{
  return (char *)jReadTypeStrings[ dataType ];
};

const char * jReadErrorStrings[]={
	"Ok",                                       // 0
...
{
	if( (error >=0 ) && (error <= 14))
	  return (char *)jReadErrorStrings[ error ];
